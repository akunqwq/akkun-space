...
阿鲲の小窝是阿鲲的个人数字空间。**本仓库开源的是「空间的实现」**——网站代码、UI 设计、工具实现、架构与技术决策，作为学习与参考。
...
> 个人空间承载私人记忆。开源实现（"how it works"），保留私人数据（"who I am"）。所有私人事件相关变量（生日、纪念日等）仅在服务端读取，永远不会出现在客户端 bundle 或 API 响应中。

如需本地运行，请复制 `.env.example` 为 `.env.local` 并填入你自己的环境变量（Supabase / B 站 WBI 等）。私人事件相关的 `PERSONAL_BIRTHDAY` / `SITE_LAUNCHED_AT` 为可选项，留空则对应事件不触发。
