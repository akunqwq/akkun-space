...
export default function MusicCard({
  ...
  const m = useMusic();
  const isCurrent = m.currentItem?.id === item.id;
  const isPlayingThis = isCurrent && m.playbackState === "playing";
  ...
  return (
    <div className="group card hover:shadow-[var(--panel-shadow)] transition-all duration-200 overflow-hidden flex flex-col">
      {/* 封面 */}
      <button
        onClick={onPlay}
        className="relative block w-full aspect-square overflow-hidden bg-[var(--theme-toggle-hover)]"
        title={`播放 ${item.title}`}
      >
        <CoverImage
          item={item}
        ...
        {isPlayingThis && (
          <span className="absolute top-1.5 right-1.5 flex items-center gap-1 text-[10px] text-white bg-[var(--btn-primary)] px-1.5 py-0.5 rounded">
            播放中
          </span>
        )}
      </button>
